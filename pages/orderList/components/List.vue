	...
	<view class="content">
		<view class="list-item" v-for="item in orderList">
			...
			<view class="item-center" @click="toDetail(item)">
				...
				<view class="note">{{item.note ? item.note : restaurantName + '欢迎您'}}</view>
				<view class="price">￥{{item.price}}</view>
			</view>
			<view class="item-bottom">
				<view class="evaluation" v-if="item.status == 3">
					<u-button type="primary" shape="circle" :plain="true" text="评价" v-if="!item.dregg"@click="toEvaluation(item)"></u-button>
					<u-button type="primary" shape="circle" :plain="true" text="查看评价" v-else @click="viewEvaluation(item)"></u-button>
				</view>
			</view>
		</view>	
	...
	export default {
		...
		methods:{
			...
			toDetail(item) {
				...
				uni.navigateTo({
				    url: `../orderDetail/orderDetail?orderDetail=${orderDetail}`,
						type:'redirectTo'
				});
			},
			// 去评价
			toEvaluation(item) {
				const orderDetail = JSON.stringify(item)
				uni.navigateTo({
				    url: `../toEvaluation/toEvaluation?orderDetail=${orderDetail}`
				});
			},
			// 查看评价
			viewEvaluation(item) {
				uni.navigateTo({
				    url: `../viewEvaluation/viewEvaluation?orderId=${item.id}`
				});
			}
		}
	}
